...
function HowItWorksContent({ gasTown }: { gasTown: GasTown }) {
  const openTerminal = () => {
    window.dispatchEvent(new CustomEvent('openGasTownTerminal'));
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        ...
        {/* Mystery button - opens build terminal easter egg */}
        <button
          onClick={openTerminal}
          className="mystery-terminal-btn"
          title="View build log"
          aria-label="Open build terminal"
        >
          <span className="mystery-terminal-icon">&gt;_</span>
        </button>
      </div>
    </div>
  );
